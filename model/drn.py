...
class DRN(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000,
                 ...
                 out_map=False, out_middle=False, pool_size=28, arch='D', input_channels=3):
...
def drn_c_26(pretrained=False, channels=3, **kwargs):
    model = DRN(BasicBlock, [1, 1, 2, 2, 2, 2, 1, 1], arch='C', input_channels=channels, **kwargs)
    pretrained_dict = model_zoo.load_url(model_urls['drn-c-26'])
    ...
    if pretrained:
        ...
        mean_weight = mean_weight.repeat(1, 5, 1, 1)
...
def drn_c_58(pretrained=False, channels=3, **kwargs):
    model = DRN(Bottleneck, [1, 1, 3, 4, 6, 3, 1, 1], arch='C', input_channels=channels, **kwargs)
    pretrained_dict = model_zoo.load_url(model_urls['drn-c-58'])
...
def drn_d_105(pretrained=False, channels=3, **kwargs):
    model = DRN(Bottleneck, [1, 1, 3, 4, 23, 3, 1, 1], arch='D', input_channels=channels, **kwargs)
    pretrained_dict = model_zoo.load_url(model_urls['drn-d-105'])
